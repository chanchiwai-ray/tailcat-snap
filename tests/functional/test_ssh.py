...
def test_ssh_remote_exec(server, client, server_workdir):
    ...
    assert out.strip() == helpers.home_dir(server), (
        "ssh remote command did not run in the server's real $HOME"
    )

    helpers.settle()
    out = helpers.lxc_exec_retry(
        client, f"timeout 15 tailcat ssh {addr} 'echo remote-exec-ok'", timeout=20
    ).output
    assert "remote-exec-ok" in out, (
        "ssh did not run an arbitrary remote command / return its output"
    )
    ...
    out = helpers.lxc_exec_retry(
        client, f"timeout 15 tailcat ssh {addr} whoami", timeout=20
    ).output
