...
def test_files_ro(server, client, server_workdir, client_workdir):
    ...
    helpers.lxc_exec_retry(client, f"timeout 15 tailcat cp {addr}:readme.txt {fetched}", timeout=20)
...
def test_files_rw(server, client, server_workdir, client_workdir):
    ...
    result = helpers.lxc_exec_retry(client, f"timeout 15 tailcat cp {upload_file} {addr}:", timeout=20)
...
def test_files_wo_recursive(server, client, server_workdir, client_workdir):
    ...
    result = helpers.lxc_exec_retry(client, f"timeout 15 tailcat cp -r {photos_dir} {addr}:photos", timeout=20)
    assert result.returncode == 0, ":wo+ server did not accept a recursive directory upload (cp -r)"
